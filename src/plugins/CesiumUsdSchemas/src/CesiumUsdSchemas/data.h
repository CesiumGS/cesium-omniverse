...
public:
  ...
  /// an invalid \p prim
  explicit CesiumData(const UsdPrim &prim = UsdPrim()) : UsdTyped(prim) {}

  /// Construct a CesiumData on the prim held by \p schemaObj .
  /// Should be preferred over CesiumData(schemaObj.GetPrim()),
  /// as it preserves SchemaBase state.
  explicit CesiumData(const UsdSchemaBase &schemaObj) : UsdTyped(schemaObj) {}
  ...
  /// \endcode
  ///
  static CesiumData Get(const UsdStagePtr &stage, const SdfPath &path);
  ...
  ///
  static CesiumData Define(const UsdStagePtr &stage, const SdfPath &path);
...
public:
  ...
  UsdAttribute CreateProjectDefaultIonAccessTokenAttr(
...
public:
  ...
  /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
  /// the default for \p writeSparsely is \c false.
  UsdAttribute CreateProjectDefaultIonAccessTokenIdAttr(
      VtValue const &defaultValue = VtValue(),
      bool writeSparsely = false) const;
...
public:
  ...
  /// | | C++ Type | double | | \ref Usd_Datatypes "Usd Type" |
  ...
  /// the default for \p writeSparsely is \c false.
  UsdAttribute
  CreateGeoreferenceOriginLongitudeAttr(VtValue const &defaultValue = VtValue(),
                                        bool writeSparsely = false) const;
...
public:
  ...
  /// the default for \p writeSparsely is \c false.
  UsdAttribute
  CreateGeoreferenceOriginLatitudeAttr(VtValue const &defaultValue = VtValue(),
                                       bool writeSparsely = false) const;
...
public:
  ...
  // --------------------------------------------------------------------- //
  /// The height of the origin in meters above the WGS84 ellipsoid. Do not
  /// confuse this with a geoid height or height above mean sea level, which can
  /// be tens of meters higher or lower depending on where in the world the
  /// origin is located.
  ///
  /// | ||
  /// | -- | -- |
  ...
  /// the default for \p writeSparsely is \c false.
  UsdAttribute
  CreateGeoreferenceOriginHeightAttr(VtValue const &defaultValue = VtValue(),
                                     bool writeSparsely = false) const;
