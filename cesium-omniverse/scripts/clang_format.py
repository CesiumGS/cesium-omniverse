...
def clang_format_on_path(clang_format_binary: str, absolute_path: Path) -> str:
    cmd = "{} -style=file {}".format(shlex.quote(clang_format_binary),
                                     shlex.quote(str(absolute_path)))
    cmd = shlex.split(cmd)
...
def clang_format_in_place(clang_format_binary: str, absolute_path: Path):
    cmd = "{} -style=file -i {}".format(shlex.quote(
        clang_format_binary), shlex.quote(str(absolute_path)))
    cmd = shlex.split(cmd)
...
def parse_args(av: List[str]):
    parser = argparse.ArgumentParser(
        description='Run / check clang-formatting.')
    parser.add_argument('--clang-format-executable',
                        help='Specific clang-format binary to use.',
                        action='store',
                        required=False
                        )
    ...
    run_type.add_argument('--fix',
                          help='Apply clang-format formatting to source in-place (destructive)',
                          action='store_true'
                          )
    run_type.add_argument('--check',
                          help='Check if source matches clang-format rules',
                          action='store_true'
                          )
    scope_type = parser.add_mutually_exclusive_group(required=True)
    scope_type.add_argument('--all',
                            help='Process all valid source files.',
                            action='store_true'
                            )
    scope_type.add_argument('--staged',
                            help='Process only staged source files.',
                            action='store_true'
                            )
...
def main(av: List[str]):
    ...
    source_files: List[utils.SourceFile] = utils.get_all_source_files(
        source_directories, args.all)
    ...
    for src in source_files:
        ...
        if args.check:
            old_text = absolute_path.read_text(
                encoding='utf-8') if not src.staged else utils.get_staged_file_text(src.relative_path)
