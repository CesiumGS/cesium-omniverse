...
def parse_args(av: List[str]):
    parser = argparse.ArgumentParser(
        description='Run / check clang-tidy on staged cpp files.')
    parser.add_argument('--clang-tidy-executable',
                        help='Specific clang-tidy binary to use.',
                        action='store',
                        required=False
                        )
...
def main(av: List[str]):
    ...
    project_root = utils.get_project_root()
    candidate_files = [f.as_posix() for f in utils.get_staged_git_files(
        project_root) if f.suffix in utils.CPP_EXTENSIONS]
